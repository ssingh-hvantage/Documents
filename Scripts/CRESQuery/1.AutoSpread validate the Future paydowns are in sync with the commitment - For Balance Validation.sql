...
Select dealname,credealid,UseRuletoDetermineNoteFunding
,FuturePayDn
,EndingBalance,FF,FuturePIKFunding
--,AdjustedTotalCommitment
,ROUND(LHS,2) LHS,ROUND(RHS_Bls ,2) RHS_Bls,
ROUND(LHS,2) - ROUND(RHS_Commit ,2) as Delta,
(CASE WHEN ROUND(LHS,2) <= ROUND(RHS_Bls ,2) THEN 'No Validation' ELSE 'Validation' end) as Vaidation_Flag
from(
	...
	,ROUND(ISNULL(tblAdComm.AdjustedTotalCommitment,0),2) as AdjustedTotalCommitment
	...
	,ROUND(ISNULL(tblAdComm.AdjustedTotalCommitment,0),2) + ISNULL(tblPIKTr.FuturePIKFunding,0)  as RHS_Commit
	from cre.deal d
