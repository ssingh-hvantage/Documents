...
Select d.DealName, d.CreDealID, n.CRENoteID, ISNULL(a.Type,b.type) as [Type],SUM(QAAmount) as QAAmount ,SUM(StgAmount) as StagAmount, 
(ISNULL(SUM(QAAmount),0)- ISNULL(SUM(StgAmount),0)) as Delta,
ABS((ISNULL(SUM(QAAmount),0)- ISNULL(SUM(StgAmount),0))) as ABS_Delta, n.ActualPayoffDate, c.Status as Calculation_Status ,le.Name as EnableM61Calculations, @AnalysisName as Scenario
from (
	Select NoteID, Type, SUM(Amount) as QAAmount,AnalysisID  
...
and ABS((ISNULL((QAAmount),0)- ISNULL((StgAmount),0))) > 0
--and ISNULL(a.Type,b.type) like 'InterestPaid' --in (@TransactionType)
--and n.CRENoteID in ('14356')
--and d.CREDealID='17-0435'
--and c.Status like '%Failed%'
Group by d.DealName,d.CreDealID, n.CRENoteID, ISNULL(a.Type,b.type), le.Name,  n.ActualPayoffDate, c.Status , n.EnableM61Calculations
